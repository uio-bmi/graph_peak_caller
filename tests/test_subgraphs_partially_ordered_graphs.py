...
class SubgraphsAndMaxPathsOnPartiallyOrderedGraphs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.scores = SparsePileup.from_intervals(self.linear_graph,
    ...
    def test_simple(self):
        ...
        pileup = SparsePileup.from_intervals(self.linear_graph,
                                             intervals)
    ...
    def test_simple2(self):
        ...
        pileup = SparsePileup.from_intervals(self.graph,
                                             intervals)
    ...
    def test_simple3(self):
        ...
        scores = SparsePileup.from_intervals(graph,
                                            [Interval(0, 5, [i]) for i in range(1, 6)])
        intervals = [Interval(0, 5, [1]),
                     Interval(0, 5, [3]),
                     Interval(0, 5, [4]),
                     Interval(0, 3, [5])]
        pileup = SparsePileup.from_intervals(graph,
                                             intervals)
        subgraphs = SubgraphCollectionPartiallyOrderedGraph.create_from_pileup(
            graph, pileup)
