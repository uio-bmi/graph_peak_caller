...
class DagHoleCleaner(object):
    ...
    def expand_hole_sides_to_right(self, positions):
        ...
        for position in positions:
            ...
            #print("Extended interval %s" % interval)
            #print("  Areas: \n   %s" % areas)
    ...
    def expand_back(self, positions):
        ...
        for position in positions:
            node = position[0]
            offset = position[1]
            interval = obg.DirectedInterval(offset, offset + 1, [node], self._graph).get_reverse()
            areas = extender.extend_interval(interval, direction=-1)
            #print("Extended interval %s back" % interval)
            #print("  Areas: \n   %s" % areas)
            self.pileup.set_area_to_value(areas, False)
